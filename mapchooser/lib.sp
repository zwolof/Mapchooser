...
int MC_GetMapvoteWinnerIndex() {
    ...
    int winner = -1;
    winner = MC_GetBiggestIntIndexFromArray(g_iVoteCount, g_alMaps_Random.Length);
    int votes = MC_GetBiggestIntFromArray(g_iVoteCount, g_alMaps_Random.Length);

    SMap map;
    g_alMaps_Random.GetArray(winner, map, sizeof(SMap));

    ChatAll("Map: \x04%s\x08 - Votes: \x04%d", map.cleanname, votes);
    // winner = biggestNumberIndex;
    ...
    //     }
    // }
    // percent = (SummarizeIntArray(g_iVoteCount, g_alMaps.Length) / g_alMaps.Length);
